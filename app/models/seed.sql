...
INSERT INTO menus (name, description, is_hot, product_id, cafe_id)
SELECT
  v.name,
  v.description,
  v.is_hot,
...
FROM (
  VALUES
    -- 커피
    ('아메리카노', '깊고 진한 에스프레소에 뜨거운 물을 더한 클래식 커피', true, '커피'),
    ('아메리카노 아이스', '시원한 물과 얼음 위에 에스프레소를 부어 깔끔하게 즐기는 커피', false, '커피'),
    ('카페 라떼', '에스프레소에 부드러운 스팀 밀크를 듬뿍 더해 고소하고 담백하게 즐기는 커피', true, '커피'),
    ('카페 라떼 아이스', '신선한 우유와 얼음, 에스프레소 샷이 어우러져 더욱 고소하고 시원한 커피', false, '커피'),
    ('바닐라 라떼', '달콤한 바닐라 시럽과 에스프레소, 부드러운 스팀 밀크가 조화로운 인기 라떼', true, '커피'),
    ('바닐라 라떼 아이스', '바닐라 시럽의 달콤함과 에스프레소의 깊은 풍미를 시원하게 즐기는 라떼', false, '커피'),
    ('카라멜 마끼아또', '달콤한 바닐라 향의 스팀 밀크에 에스프레소 샷과 카라멜 소스를 얹은 부드러운 커피', true, '커피'),
    ('카라멜 마끼아또 아이스', '바닐라 시럽과 우유, 얼음 위에 에스프레소 샷과 카라멜 소스를 더한 시원하고 달콤한 커피', false, '커피'),
    -- 라떼
    ('그린티 라떼', '쌉쌀한 국내산 녹차와 부드러운 우유가 만나 건강하고 든든한 논커피 라떼', true, '라떼'),
    ('그린티 라떼 아이스', '진한 녹차의 풍미를 신선한 우유와 얼음으로 더욱 시원하고 깔끔하게 즐기는 라떼', false, '라떼'),
    ('초코 라떼', '진한 초콜릿과 부드러운 스팀 밀크가 만나 달콤하고 따뜻하게 몸을 녹여주는 음료', true, '라떼'),
    ('초코 라떼 아이스', '진하고 달콤한 초콜릿을 신선한 우유와 얼음으로 시원하게 즐기는 음료', false, '라떼'),
    ('고구마 라떼', '달콤하고 구수한 국내산 고구마와 따뜻한 우유가 만나 든든한 한 끼 식사 대용 음료', true, '라떼'),
    ('생강 라떼', '알싸한 생강과 부드러운 우유가 만나 몸을 따뜻하게 데워주는 건강 라떼', true, '라떼'),
    ('미숫가루 라떼', '18가지 곡물로 만들어 고소하고 든든하며 시원하게 즐기는 한국식 건강 음료', false, '라떼'),
) AS v(name, description, is_hot, product_name)
...
FROM (
  VALUES
    ...
    ('생강 라떼'),
    ('미숫가루 라떼'),
) AS v(name, product_name)
JOIN menus m ON v.name = m.name
WHERE m.cafe_id = '1f9a2b3c-4d5e-6f7a-8b9c-0d1e2f3a4b5c';

-- 레시피(Recipes) 조리 단계 업데이트
-- 커피
UPDATE recipes SET steps = ARRAY['뜨거운 잔을 준비합니다.', '에스프레소 2샷(30ml)을 추출합니다.', '추출된 샷에 뜨거운 물(200ml)을 붓습니다.'] WHERE name = '아메리카노';
UPDATE recipes SET steps = ARRAY['얼음을 가득 채운 아이스 잔을 준비합니다.', '차가운 물(180ml)을 붓습니다.', '에스프레소 2샷(30ml)을 추출하여 물 위에 붓습니다.'] WHERE name = '아메리카노 아이스';
UPDATE recipes SET steps = ARRAY['뜨거운 잔을 준비합니다.', '잔에 에스프레소 2샷(30ml)을 추출합니다.', '우유(250ml)를 스팀 피처에 담아 벨벳 질감으로 스티밍합니다.', '샷 위에 스팀 우유를 부드럽게 부어 하트 모양을 만듭니다.'] WHERE name = '카페 라떼';
UPDATE recipes SET steps = ARRAY['얼음을 가득 채운 아이스 잔을 준비합니다.', '잔에 우유(200ml)를 먼저 붓습니다.', '에스프레소 2샷(30ml)을 추출하여 우유 위에 부드럽게 붓습니다.'] WHERE name = '카페 라떼 아이스';
UPDATE recipes SET steps = ARRAY['뜨거운 잔에 바닐라 시럽(2펌프)을 넣습니다.', '에스프레소 2샷(30ml)을 추출하여 시럽과 잘 섞어줍니다.', '우유(220ml)를 곱게 스티밍하여 잔에 붓습니다.'] WHERE name = '바닐라 라떼';
UPDATE recipes SET steps = ARRAY['아이스 잔에 바닐라 시럽(2펌프)과 에스프레소 2샷(30ml)을 넣고 섞어줍니다.', '우유(200ml)를 붓고 얼음을 가득 채웁니다.'] WHERE name = '바닐라 라떼 아이스';
UPDATE recipes SET steps = ARRAY['뜨거운 잔에 바닐라 시럽(2펌프)을 넣습니다.', '우유(220ml)를 곱게 스티밍하여 잔에 붓습니다.', '에스프레소 2샷(30ml)을 추출하여 우유 거품 위에 점을 찍듯이 붓습니다.', '카라멜 소스를 격자 모양으로 드리즐합니다.'] WHERE name = '카라멜 마끼아또';
UPDATE recipes SET steps = ARRAY['아이스 잔에 바닐라 시럽(2펌프)을 넣습니다.', '우유(200ml)를 붓고 얼음을 가득 채웁니다.', '에스프레소 2샷(30ml)을 추출하여 얼음 위에 부드럽게 붓습니다.', '카라멜 소스를 격자 모양으로 드리즐합니다.'] WHERE name = '카라멜 마끼아또 아이스';

-- 라떼
UPDATE recipes SET steps = ARRAY['뜨거운 잔에 녹차 파우더(20g)와 설탕(10g)을 넣습니다.', '뜨거운 물(30ml)을 부어 파우더가 뭉치지 않게 잘 풀어줍니다.', '우유(220ml)를 곱게 스티밍합니다.', '녹차 베이스에 스팀 우유를 부으면서 잘 저어줍니다.'] WHERE name = '그린티 라떼';
UPDATE recipes SET steps = ARRAY['잔에 녹차 파우더(20g)와 설탕(10g), 우유(220ml)를 넣고 잘 저어줍니다.', '얼음을 가득 채웁니다.'] WHERE name = '그린티 라떼 아이스';
UPDATE recipes SET steps = ARRAY['뜨거운 잔에 초코 파우더(30g)를 넣습니다.', '뜨거운 물 또는 우유(30ml)를 약간 부어 잘 녹여 페이스트로 만듭니다.', '우유(220ml)를 부드럽게 스티밍합니다.', '초코 페이스트에 스팀 우유를 부으면서 잘 저어줍니다.'] WHERE name = '초코 라떼';
UPDATE recipes SET steps = ARRAY['잔에 초코 파우더(30g)와 우유(220ml)를 넣고 잘 저어줍니다.', '얼음을 가득 채웁니다.', '기호에 따라 휘핑 크림을 올립니다.'] WHERE name = '초코 라떼 아이스';
UPDATE recipes SET steps = ARRAY['뜨거운 잔에 고구마 페이스트(40g)를 넣습니다.', '우유(250ml)를 스티밍합니다.', '고구마 페이스트에 스팀 우유를 부으면서 덩어리가 없도록 잘 저어줍니다.', '기호에 따라 견과류 토핑을 올립니다.'] WHERE name = '고구마 라떼';
UPDATE recipes SET steps = ARRAY['뜨거운 잔에 생강청(30g)을 넣습니다.', '우유(250ml)를 스티밍합니다.', '생강청에 스팀 우유를 부으면서 잘 저어줍니다.', '시나몬 파우더를 살짝 뿌려 제공합니다.'] WHERE name = '생강 라떼';
UPDATE recipes SET steps = ARRAY['블렌더에 미숫가루(40g), 우유(250ml), 꿀(15ml), 얼음(8개)을 넣습니다.', '내용물이 부드러워질 때까지 약 30초간 블렌딩합니다.', '잔에 옮겨 담아 제공합니다.'] WHERE name = '미숫가루 라떼';
...
FROM (
  VALUES
    ...
    ('아메리카노', '뜨거운 물', '200ml'),
    ('아메리카노 아이스', '얼음', '가득'),
    ('아메리카노 아이스', '차가운 물', '180ml'),
    ('아메리카노 아이스', '에스프레소', '2샷(30ml)'),
    ('카페 라떼', '에스프레소', '2샷(30ml)'),
    ('카페 라떼', '우유', '250ml'),
    ('카페 라떼 아이스', '얼음', '가득'),
    ('카페 라떼 아이스', '우유', '200ml'),
    ('카페 라떼 아이스', '에스프레소', '2샷(30ml)'),
    ('바닐라 라떼', '바닐라 시럽', '2펌프'),
    ('바닐라 라떼', '에스프레소', '2샷(30ml)'),
    ('바닐라 라떼', '우유', '220ml'),
    ('바닐라 라떼 아이스', '바닐라 시럽', '2펌프'),
    ('바닐라 라떼 아이스', '에스프레소', '2샷(30ml)'),
    ('바닐라 라떼 아이스', '우유', '200ml'),
    ('바닐라 라떼 아이스', '얼음', '가득'),
    ('카라멜 마끼아또', '바닐라 시럽', '2펌프'),
    ('카라멜 마끼아또', '우유', '220ml'),
    ('카라멜 마끼아또', '에스프레소', '2샷(30ml)'),
    ('카라멜 마끼아또', '카라멜 소스', '드리즐'),
    ('카라멜 마끼아또 아이스', '바닐라 시럽', '2펌프'),
    ('카라멜 마끼아또 아이스', '우유', '200ml'),
    ('카라멜 마끼아또 아이스', '얼음', '가득'),
    ('카라멜 마끼아또 아이스', '에스프레소', '2샷(30ml)'),
    ('카라멜 마끼아또 아이스', '카라멜 소스', '드리즐'),

    -- 라떼
    ('그린티 라떼', '녹차 파우더', '20g'),
    ('그린티 라떼', '설탕', '10g'),
    ('그린티 라떼', '우유', '250ml'),
    ('그린티 라떼 아이스', '녹차 파우더', '20g'),
    ('그린티 라떼 아이스', '설탕', '10g'),
    ('그린티 라떼 아이스', '우유', '220ml'),
    ('그린티 라떼 아이스', '얼음', '가득'),
    ('초코 라떼', '초코 파우더', '30g'),
    ('초코 라떼', '우유', '250ml'),
    ('초코 라떼 아이스', '초코 파우더', '30g'),
    ('초코 라떼 아이스', '우유', '220ml'),
    ('초코 라떼 아이스', '얼음', '가득'),
    ('고구마 라떼', '고구마 페이스트', '40g'),
    ('고구마 라떼', '우유', '250ml'),
